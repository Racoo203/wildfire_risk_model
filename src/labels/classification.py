"""Post-hoc cleaning of ambiguous 'Low' risk labels via pairwise k-means.

Rationale (from the source paper, Sec. 3.2.3): pixels labeled Low purely
because no historical fire was recorded there are not necessarily
low-risk — some share feature-space characteristics with higher-risk
pixels and are more likely mislabeled than genuinely safe. For each
higher-risk class, k-means (k=2) is run on a (Low, high_class) pair in
the 14-factor feature space; Low pixels that land in the cluster whose
centroid is dominated by the high-risk class are flagged as ambiguous
and removed (set to NaN) rather than kept as false negatives.
"""
...
class LabelCleaner(VarBuilder):
    ...
    def clean(
        ...
        """
        Run the primary (paper-matching) k-means cleaning pass, write the
        cleaned label raster, and — if enabled in config — run a sensitivity
        analysis against the primary result.

        Returns:
            Cleaned label array, same shape as `labels`, with flagged Low
            pixels set to NaN.
        """
        out_paths = {
    ...
    def _flag_low_pixels(
        ...
        *,
        ...
        """
        For each high-risk class, subsample (Low, high_class) pairs, run
        k-means (k=2) in standardized feature space, and flag Low pixels
        that fall into the cluster dominated by the high-risk class.

        `feature_weights`, if given, rescales standardized features before
        clustering — used by the sensitivity analysis to test robustness
        to light feature reweighting without duplicating this method.

        Returns:
            Set of flat-array indices (into flat_labels) flagged for removal.
        """
    ...
    def sensitivity_analysis(
        ...
        """
        Re-run `_flag_low_pixels` under alternate n_init values, alternate
        random seeds, and light feature reweighting, and compare each
        variant's flagged set to the primary run.

        "Variance" here is the symmetric-difference fraction relative to
        the Low-class sample size — i.e. what fraction of Low pixels were
        flagged inconsistently between the baseline and the variant. The
        paper reports <0.2% variance across its own sensitivity checks;
        this reproduces that comparison for the Essex data.

        Returns:
            DataFrame of one row per variant, also saved to
            figures/label_cleaning_sensitivity_<season>.csv.
        """
        cfg = self.config["labels"]
        max_sample = cfg["max_sample_per_class"]
    ...
    def _build_sensitivity_variants(cfg: dict) -> List[dict]:
        """Assemble the list of (n_init, seed, reweight) variants to test."""
    ...
    def _flatten_valid(labels: np.ndarray, feature_arrays: Dict[str, np.ndarray]):
        """Flatten labels/features to 1-D arrays over valid (non-NaN label) pixels."""
    ...
    def _subsample(idx: np.ndarray, max_n: int, rng: np.random.Generator) -> np.ndarray:
        """Randomly subsample an index array, reproducibly, via the given Generator."""
        if len(idx) <= max_n:
            return idx
    ...
    def _apply_flags(
        ...
        """Set flagged Low pixels to NaN and reshape back to the original raster grid."""
