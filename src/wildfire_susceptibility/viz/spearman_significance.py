...
def _one_hot_encode(
    ...
    Dummies are cast to float64 (pandas' get_dummies default is bool) --
    not load-bearing for the crash above (which reproduces on plain
    float64 arrays too), but keeps every column in `encoded_df` a uniform
    numeric dtype. The source column is cast to nullable Int64 first
    (raster-derived columns like landuse_class arrive as float64) so dummy
    names read "landuse_class_3" not "landuse_class_3.0" -- the same
    cosmetic fix DatasetPrep.fit_categorical_encoding already applies for
    the same reason (dataset_prep.py).
    """
    out = df.copy()
...
def compute_spearman_significance_export(
    ...
    """
    encoded_df, parent_map = _one_hot_encode(df, categorical_cols)
